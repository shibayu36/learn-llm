...
「日本の首都は」の次の文字を当てるには、「は」の位置に「日本の首都の話をしている」という情報が要ります。ところが1.4までの処理では、「は」の位置のベクトルには「自分は『は』という文字で、5番目にいる」という情報しか入っていません。前にある「日本」「首都」の中身は、まだどこにも取り込まれていません。

「は」の位置へ前の位置の情報を運ぶ処理が必要です。単純な方法は前の位置のベクトルをすべて平均して足すことですが、これでは「の」も「首都」も同じ割合で混ざります。「は」に必要なのは「首都」「日本」であって「の」ではなく、しかも何が必要かは文によって変わります。

そこでSelf-Attentionを使います。**Self-Attentionは、各位置が他の位置の情報をどの割合で取り込むかを入力に応じて決め、その割合でベクトルを混ぜる仕組みです。** 参照する側も参照される側も同じ入力列なので、Self-Attentionと呼びます。

### Q・K・Vは文の中で何を表すか

「どの割合で取り込むか」を入力から決めるために、各位置のベクトルから3種類のベクトルを作ります。

- **Query（Q）** は、その位置が今どんな情報を探しているかを表します。「は」のQueryは「直前で話題になっている名詞が欲しい」という内容になります。
- **Key（K）** は、その位置がどんな情報を持っているかを表します。探す側から見たときの目印です。「都」のKeyは「場所や地名に関する文字」という内容になります。
- **Value（V）** は、選ばれたときに実際に相手へ渡す中身です。

QueryとKeyの内積を取ると、「探しているもの」と「持っているもの」がどれだけ噛み合うかが1つの数になります。内積は対応する成分を掛けて足す計算で、2つのベクトルの向きが揃うほど大きくなります。「は」のQueryと各位置のKeyの内積を並べると、「は」から見た各位置のスコア（score）ができます。数値は説明用です。

```text
「は」のQueryと、各位置のKeyの内積          softmaxで割合にする
...
softmaxは、スコアの並びを「合計が1の割合」へ変換します。スコアが大きい位置ほど大きな割合になり、小さい位置はほぼ0になります。この割合をAttention weightと呼びます。上の例では「は」は「首都」を7割、「日本」を3割の割合で参照し、「の」はほとんど参照しません。

操作デモ：[内積が参照の割合に変わる過程を見る](demos/part1.html#dot-product)

Queryの向きと長さを変え、スコアとsoftmax後の割合がどう動くかを確かめられます。

最後に、この割合で各位置のValueを混ぜます。

```text
「は」の新しいベクトル
  = 0.21 × V(日) + 0.08 × V(本) + 0.01 × V(の)
  + 0.34 × V(首) + 0.34 × V(都) + 0.03 × V(は)
```

混ぜた結果が「は」の位置の新しいベクトルです。「首都」「日本」のValueが大きな割合で入っているので、このベクトルは「日本の首都の話をしている『は』」を表すようになります。1.4までは「は」という文字と位置しか持っていなかったベクトルに、前の文字の情報が取り込まれました。

操作デモ：[ValueをAttention weightの分だけ縮めて足す](demos/part1.html#weighted-sum)

矢印を1本ずつつなぎ、「他のtokenの情報を集める」がベクトルの足し算であることを確かめられます。

Q・K・Vを別々に作るのは、「自分が何を持っているか」と「自分が何を探しているか」が別だからです。「は」は名詞ではありませんが、名詞を探します。1.4までのベクトルどうしをそのまま内積すると、自分と似た文字しか見つけられません。探すものをQuery、探されるときの目印をKey、渡す中身をValueと分けることで、それぞれに適した変換を学習できます。

各ベクトルは、1.4までのベクトル `X` を線形変換して作ります。線形変換は `X` の成分を混ぜ合わせて新しい成分を作る計算で、どの成分をどれだけ混ぜるかを `D × D` の数の表が決めます。この表がWq・Wk・Wvです。表の中身は人が決めるのではなく、次token予測の誤差から更新されます。学習が進むと、「は」のQueryが名詞のKeyと噛み合うような表になっていきます。

### 全位置の計算を1つの行列にまとめる

ここまでは「は」の位置だけを見てきましたが、同じ計算をすべての位置で行います。各位置のQueryと各位置のKeyの内積を全部並べると、`T × T` の表になります。行が探す側の位置、列が探される側の位置です。
...
（各位置の     本   ·      ·      ·      ·      ·      ·
  Query）      の   ·      ·      ·      ·      ·      ·
               首   ·      ·      ·      ·      ·      ·
               都   ·      ·      ·      ·      ·      ·
               は  2.0    1.0   -1.0    2.5    2.5    0.0    ← 上で計算した「は」の行のスコア
```

行ごとにsoftmaxをかけると、各行が合計1のAttention weightになります。この表と各位置のValueを掛けると、全位置の新しいベクトルが一度に求まります。

この計算を式で書くと次のようになります。バイアスは省略しています。

```text
Q = XWq,  K = XWk,  V = XWv

scores  = QKᵀ / √d_k        全位置のQueryと全位置のKeyの内積の表 [T, T]
weights = softmax(scores)    行ごとに合計1の割合にする
output  = weights V          割合でValueを混ぜる [T, D]
```

`QKᵀ` は、上の `T × T` の表を行列積で一度に計算する書き方です。`d_k` はKeyベクトルの成分数で、1ヘッドでは `d_k = D` です。バッチのB個の系列は互いに混ざらず、それぞれ独立にこの計算をします。

`√d_k` で割るのは、softmaxが1か所に偏りすぎるのを防ぐためです。内積は成分数Dが増えるほど足し合わせる項が増え、値の幅が広がります。幅が広いままsoftmaxへ入れると、1つの位置だけが割合をほぼ独占します。√Dで割るとDによらず同じくらいの幅に収まります。この計算をscaled dot-product attentionと呼びます。上の「は」の例のスコアは、この割り算を済ませた後の値と考えてください。
...
`nn.Linear(d_model, d_model)` が、本文のWq・Wk・Wvにあたる `D × D` の表を持ちます。`query.weight`・`key.weight`・`value.weight` が学習で更新されるパラメータです。PyTorchは `x @ weight.T + bias` を計算するので、保存される `weight` の軸の順序は式のWqとは逆です。
