...
説明用の例と記号を吟味する。入力の「B」とbatch sizeの「B」のような衝突を避け、文字・token・IDなど混同しやすいものを具体例で区別する。未導入の用語を前提に説明しない。用語と表記は `docs/GLOSSARY.md` に従い、新しい用語を導入したら用語集へ追加する。
...
通常の手順では不要な設定を考えさせない。仕組みの理解が目的のあいだはCPU固定とし、device選択やGPU対応のコードを載せない。速度が問題になった時点で、速くしたい人向けの節として別に出す。読者が変えない値はConfigに置かない。
