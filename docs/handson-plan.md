...
各節は、節の冒頭から「実装」までを見出しなしの散文で書く。散文の流れは次の順を目安にするが、この順を見出しにはしない。見出しにすると説明が枠に引きずられ、目的が抜けたり説明が細切れになったりするため。

1. その技術が何のために必要か。何を表したいのか、何ができるようになりたいのか
2. 今の部品では何が足りないのか
3. 仕組み。最小限の式・shape・具体例・図を含める

節の中で見出しを置くのは、読者が読む・書く・確かめるを切り替える目印になる次の3つだけとする。

- 実装
- 動作確認、または実験（理解を深めるために有効な場合のみ）
- この節で理解したこと

説明が長くなり戻り読みしにくい節では、内容固有の見出し（「Q・K・Vは何を表すか」のように、見出し自体が主張になるもの）を散文の中に足してよい。「仕組み」「問題」のような汎用の見出しは使わない。

各技術は必要になる理由から説明する。数式は具体例・Tensorのshape・コードと結び付ける。

節を書き始める前に「この節で読者に理解させたい1つのこと」を決め、そこから本文と最小のコードを逆算する。ファイル構成や見出しの型を先に作って中身を埋める順にしない。Part 1ではその順で書いたため、構造が理解を邪魔する箇所を後から大量に直すことになった。
...
説明用の例と記号を吟味する。入力の「B」とbatch sizeの「B」のような衝突を避け、文字・token・IDなど混同しやすいものを具体例で区別する。未導入の用語を前提に説明しない。新しい記号・定数・特殊トークンは、登場する場所で意味を説明する。データを作るコードには、実際に出力されるデータの例を添える。用語と表記は `docs/GLOSSARY.md` に従い、新しい用語を導入したら用語集へ追加する。
...
本文は読者がGPTを理解し再現するためのものなので、次のものは本文に書かない。

- この計画にある制作基準そのもの。型注釈の付け方、コードの分け方、説明の順序、実測値と説明用の数値を区別する方針などは、書き方に反映するものであり、読者に宣言するものではない
- 想定読者が既に知っている記法やツールの説明。型注釈の読み方、`if __name__ == "__main__"`、importの別名、uvの一般的な説明など
- 作り手の事情。後のPartでの実装予定、データ取得方式を変えた経緯、確認コードの値を選んだ理由など
- 誰も抱かない誤解を否定する断り書き（「〜という意味ではありません」）や、コアを支えない一般論の前置き
- 実験の目的を言い換えて2回書くこと。「何を変え、何を観察するか」は1回で示す

これらは制作基準を型どおり当てはめたときに紛れ込みやすい。文章レビューの「読者に不要な情報」の観点で毎回確認する。

### コードと実行手順

**コードはファイル単位ではなく、理解する単位で載せる。** 各節には、その節の概念を理解し、動作確認で試せる最小の実装だけを置く。後の節で初めて使う関数・引数・importは、使う節まで出さない。読者が書いたコードは、その節の動作確認で必ず呼ばれるようにする。同じファイルが複数の節に分かれて書かれてよく、その場合は「この節のコードを順に保存する」と明記する。節を書くときは、まず動作確認で何を試すかを決め、そこから必要な実装を逆算する。

動作確認・実験の入口は、作業用ディレクトリ直下の `1_2.py`・`1_3.py` のように節番号に対応するファイルへ分ける。各ファイルには必要なimport・準備処理・確認コードを載せ、前節の確認ファイルや変数には依存させない。同じ節の中では、確認する内容に応じて末尾へ追記してよい。`tiny_gpt/` の部品は共通で段階的に実装し、最後の学習は `1_13.py` から呼ぶ。部品は更新されるため、過去の確認ファイルを残しても当時の実装や出力は保存されない。節ごとの再現性が必要になったら、部品の版も保存する方法を検討する。

確認コードは、その節で何を観察して何が分かるかを優先する。モデル内部の式を再計算して一致を見るだけでなく、たとえば位置情報なら「同じ文字でも位置が違えば別のベクトルになる」という変化を観察できる確認にする。

例外はモデル本体の `model.py` だけとする。読者がファイル全体の形を先に把握できるように、1.3で完成時の構造（クラス名・入出力の型・担当する節）を空実装として先に出す。入力と同じshapeを返す部品は入力をそのまま返す空実装にして、`TinyGPT` の `__init__`・`forward` は1.3から完成形にする。Attention weightも返すAttention系だけ `NotImplementedError`、自作クラスでないLM Headだけ `nn.Identity` にする。中身は使う節まで出さない。この例外を他のファイルへ広げない。

既存コードへの変更は、クラス・関数などの囲みと配置の目印になる既存行を残し、変更しない部分を `...` で省略する。**マーカーは変更範囲が分かる最小限にし、次の単位で使い分ける。**

- 独立した追加項目が1〜数個なら `# ← 追加` を付ける。import一覧の名前やクラスの属性などが該当する。複数行の式や短い `if` を単独で追加する場合も、先頭行に1つだけ付ける。
- 連続する処理を1つの塊として追加するなら、先頭に `# --- 追加: <配置> ---` を1つ置く。その塊の各行にはマーカーを重ねない。たとえば入力長の取得・範囲の確認・位置番号の作成は、まとめて1つの塊として示す。
- 既存部分の変更には `# --- 差し替え: <対象> ---` を付ける。多数の追加項目が散在する場合も、行末マーカーを並べず、対象部分の全体を差し替えバナーで示す。

バナーの直後は1行空け、計算を説明するコメントと区別する。関数・クラス全体を置き換えるときは本文で対象を指定して全文を載せ、バナーは付けない。同じファイルの同じ目的の変更は、importを含めて1つのコードブロックにまとめる。独立した関数の追加や末尾への追記は、本文だけで配置が伝わるならバナーを省く。省略記号と変更マーカーは教材上の目印とし、`sample/` の完成コードには入れない。

他言語の経験者が追えるPythonにする。リスト内包表記・ジェネレータ式などを避け、素直なループを使う。コードは責務で分け、重要な計算が構造から読めるようにする。汎用Trainer、テストのためだけのinterface・mock、将来のための過剰な抽象化は作らない。切り替える予定のない引数（backendの選択など）や、クラスの外に出す理由のない補助関数も作らない。

本筋（Transformerの理解）でない部品は、最も単純な実装を選ぶ。TokenizerをBPEから1文字単位にしたのはこの判断による。
...
コメントは計算の理解を助けるために使う。Tensorの軸、1tokenずらす理由、未来を隠す理由、勾配計算と更新の違いなどを補う。コードを読み上げるだけのコメントや、作業履歴・会話の経緯は書かない。コメントも初心者が読んで意味の通る言葉で書く。「`[B, T]` のそれぞれについて表の行を取り出す。結果は `[B, T, D]`」のようにshapeだけを述べるコメントは、本文で意味を説明していても読者には伝わらない。

`sample/` は参照用の完成コードとし、読者は別の作業場所で実装する。本文を順に進めると完成コードと同じ実装へ到達できるようにし、両者の一致を確認する。前のPartの実装を再利用し、章ごとの差分を追える単位で拡張する。

通常の手順では不要な設定を考えさせない。仕組みの理解が目的のあいだはCPU固定とし、device選択やGPU対応のコードを載せない。速度が問題になった時点で、速くしたい人向けの節として別に出す。読者が変えない値はConfigに置かない。入力が決まっているデータへの防御的なvalidation、CPUスレッド数の設定、実行時間の計測コードも載せない。所要時間は「5〜10分程度」くらいの粒度で書き、環境ごとの場合分けはしない。

実行手順のコマンドは、読者が自分の作業ディレクトリで実行する前提で検証する。`sample/` の中でしか動かないコマンド（`--directory sample` など）を本文に書かない。
...
Part 1の推敲で繰り返し直した点。上の基準を型どおり当てはめても紛れ込むので、節ごとに次を確認する。頻度の高い順に並べている。

1. 式・shape・手順の列挙で終わっていないか。その計算が何を意味するかを、具体例か図で先に示しているか
2. 学習に関係ないコード・記述が混ざっていないか。validation、device、性能計測、環境ごとの場合分け
3. その節で使わない関数・引数・importを先に出していないか。読者がその節で何かを実装して動かせるか
4. 節の冒頭で「この技術は何のためにあるか」を述べているか。見出しの型を埋めただけになっていないか
5. 上から順に読んだ人に意味が通じない記号・定数・コメントがないか。データを作るコードに出力例があるか
6. 切り替えない引数、理由のない分割、ファイルごとのmainなど、不要な構造を作っていないか
7. 制作基準や作り手の事情が本文に漏れていないか
8. 用語が `docs/GLOSSARY.md` と一致しているか
9. 手順のコマンドが読者の作業ディレクトリで動くか
