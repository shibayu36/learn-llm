...
説明では「なぜ必要か」を先に示す。コードだけを長く貼らず、何を作るかとコードの対応を明示する。実験は「何が変わったか」「考えられる理由」「その結果だけでは断言できないこと」を扱う。

冒頭にGPT全体のアーキテクチャ図を置く。節ごとの実験は必須にしない。図・数値例・説明で分かるものはその場で終え、実験は何を理解するための比較かを先に示す。配線やshapeの正しさを確かめる動作確認と、概念を理解するための実験を区別する。
...
MermaidなどMarkdownで表現できる図は本文に残す。Part 1の操作デモは `docs/demos/part1.html` の1ファイルへまとめ、本文の各箇所からフラグメントでリンクする。HTMLは外部依存なしで開ける構成にする。

`core-message-writing` と `japanese-sentence-style` を執筆・推敲の基準にする。読者の理解に必要な具体を残し、一般論や同じ説明の重複を減らす。

制作状況・未計測など作成側の事情は本文へ混ぜず、進捗資料へ記録する。所要時間は読者が判断できる実行条件と目安を示す。文字の例とshapeの記号が衝突しないようにし、未導入用語や同じ記号の多義性も見直す。

コードは責務で分け、重要な計算が構造から読めるようにする。汎用Trainer、テストのためだけのinterface・mock、将来のための過剰な抽象化は作らない。高レベルなTransformer実装は使わず、Q/K/V・スコア・Mask・softmax・ヘッド分割・KV Cache・loss maskingを明示する。Tensor・Linear・Embedding・optimizerなどの基本部品は利用する。

Part 1のBlockを再利用して拡張する。章ごとの差分が追える単位で変更し、参照用の完成コードと本文の一致を確認する。

すべてのサンプルコードにPythonの型を付ける。関数の引数・戻り値と設定・コレクションの型を明示し、本文のコード例にも揃えて反映する。Tensorのshapeは型とは別に説明する。

コメントは公開API・背景・複雑な計算・大きなファイルの区切りに加え、読者がコードの挙動を理解するために必要な箇所へ付ける。Tensorの軸、具体的な入出力対応、未来を隠す理由、勾配計算と更新の違いなどを補う。作業履歴や会話の経緯は書かない。
